    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.properties.update", [$property->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('property_title') ? 'is-invalid' : '' }}" type="text" name="property_title" id="property_title" value="{{ old('property_title', $property->property_title) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type_id" id="type_id" required>
                    @foreach($types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('type_id') ? old('type_id') : $property->type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rooms') ? 'is-invalid' : '' }}" type="number" name="rooms" id="rooms" value="{{ old('rooms', $property->rooms) }}" step="1" required>
                @if($errors->has('rooms'))
                    <div class="invalid-feedback">
                        {{ $errors->first('rooms') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('property_price') ? 'is-invalid' : '' }}" type="number" name="property_price" id="property_price" value="{{ old('property_price', $property->property_price) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('per') ? 'is-invalid' : '' }}" name="per" id="per" required>
                    ...
                    @foreach(App\Models\Property::PER_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('per', $property->per) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('per'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('area') ? 'is-invalid' : '' }}" type="number" name="area" id="area" value="{{ old('area', $property->area) }}" step="1" required>
                @if($errors->has('area'))
                    <div class="invalid-feedback">
                        {{ $errors->first('area') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('property_video') ? 'is-invalid' : '' }}" type="text" name="property_video" id="property_video" value="{{ old('property_video', $property->property_video) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags[]" id="tags" multiple>
                    @foreach($tags as $id => $tag)
                        <option value="{{ $id }}" {{ (in_array($id, old('tags', [])) || $property->tags->contains($id)) ? 'selected' : '' }}>{{ $tag }}</option>
                    @endforeach
                </select>
                @if($errors->has('tags'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('available') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="available" id="available" value="1" {{ $property->available || old('available', 0) === 1 ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('feature_property') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="feature_property" id="feature_property" value="1" {{ $property->feature_property || old('feature_property', 0) === 1 ? 'checked' : '' }}>
